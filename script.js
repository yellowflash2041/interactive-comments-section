  ...
  comments.forEach((item) => {
    ...
    if (item.replies.length != 0) {
      ...
      <div class="inner__comment">
        ...
        <div class="comment__text">
          ${detectTag(reply.content, reply.replyingTo)}
        </div>
      </div>
      </div>
    </div>`;
...
function detectTag(text, replyingTo) {
  let inputSplit = text.split(" ");
  let i = 0;
  for (; inputSplit[i] === '' && i < inputSplit.length; i++);
  if (replyingTo && !inputSplit[i].includes("@")) {
    inputSplit.unshift(`@${replyingTo}`);
  }
  for (i = 0; i < inputSplit.length; i++) {
    if (inputSplit[i].includes("@") && inputSplit[0].trim().length > 0) {
      inputSplit[i] = `<span class='comment__replyingTo'>${inputSplit[i]}</span>`;
    }
  }
  ...
  for (comment of comments) {
    ...
    for (reply of comment.replies) {
      if (reply.id === elementToEditId) {
        ...
        document.querySelector(
          `[data-id='${elementToEditId}'] .inner__comment .comment__text`
        ).innerHTML = detectTag(reply.content, reply.replyingTo);
      }
    }
  }
  buildDeleteBtn();
